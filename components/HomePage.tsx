...
export function HomePage() {
  ...
  const reveal = useMemo(
    () => ({
      ...
      transition: { duration: shouldReduceMotion ? 0.01 : isMobile ? 0.42 : 0.7, ease: 'easeOut' }
    }),
  ...
  const heroMotion = useMemo(
    () => ({
      ...
      transition: { duration: shouldReduceMotion ? 0.01 : isMobile ? 0.38 : 0.6, ease: 'easeOut' }
    }),
    [isMobile, shouldReduceMotion]
  );
    ...
    <div className="relative overflow-hidden mobile-premium-shell">
      ...
      <main>
        <section id="hero" className="relative overflow-hidden bg-hero">
          <div className="hero-orb left-[-7rem] top-[6%] h-52 w-52 bg-primary/18 md:left-[-6rem] md:h-72 md:w-72 md:bg-primary/30" />
          {!isMobile ? <div className="hero-orb ambient-glow right-[-5rem] top-14 h-80 w-80 bg-secondary/25" /> : null}
          {!isMobile ? <div className="hero-orb bottom-10 left-[34%] h-72 w-72 bg-accent/18" /> : null}
          {!isMobile ? <div className="ambient-glow pointer-events-none absolute left-[12%] top-[20%] h-56 w-56 rounded-full bg-sky-300/10 blur-[120px]" /> : null}
          <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_28%_30%,rgba(255,255,255,0.04),transparent_18%),linear-gradient(180deg,rgba(2,6,23,0)_0%,rgba(2,6,23,0.38)_100%)] md:bg-[radial-gradient(circle_at_28%_30%,rgba(255,255,255,0.08),transparent_20%),linear-gradient(180deg,rgba(2,6,23,0)_0%,rgba(2,6,23,0.3)_100%)]" />
          ...
          <div className="section-container relative flex min-h-[100svh] items-center py-14 md:min-h-[96vh] md:py-28 lg:py-32">
            <div className="grid w-full items-center gap-10 md:gap-14 lg:grid-cols-[1.08fr_0.92fr] lg:gap-14">
              <div className="relative z-10">
                ...
                <motion.h1
                  {...heroMotion}
                  transition={{ ...heroMotion.transition, delay: shouldReduceMotion ? 0 : 0.06 }}
                  className="hero-title mt-6 max-w-5xl font-heading text-[2.8rem] font-semibold leading-[0.9] tracking-[-0.06em] text-white md:mt-8 md:text-[5rem] xl:text-[6rem]"
                >
                  <span className="block text-white md:drop-shadow-[0_0_24px_rgba(125,211,252,0.14)]">{t.heroTitleLead}</span>
                  <span className="gradient-text gradient-flow block pb-2 md:pb-3 md:drop-shadow-[0_0_34px_rgba(56,189,248,0.22)]">
                    {t.heroTitleAccent}
                  </span>
                </motion.h1>
                <motion.p
                  {...heroMotion}
                  transition={{ ...heroMotion.transition, delay: shouldReduceMotion ? 0 : 0.12 }}
                  className="body-copy mt-6 max-w-2xl text-base leading-7 text-slate-200/84 md:mt-9 md:text-xl md:leading-9"
                >
                  {t.tagline}
                </motion.p>
                ...
                <motion.div
                  {...heroMotion}
                  transition={{ ...heroMotion.transition, delay: shouldReduceMotion ? 0 : 0.18 }}
                  className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap md:mt-11 md:gap-4"
                >
                ...
                <motion.div
                  {...heroMotion}
                  transition={{ ...heroMotion.transition, delay: shouldReduceMotion ? 0 : 0.24 }}
                  className="mt-10 grid max-w-2xl gap-3 sm:grid-cols-2 md:mt-14 md:gap-4"
                >
                  {[t.heroMetricOne, t.heroMetricTwo].map((item) => (
                    <div key={item} className="glass-card rounded-[22px] px-5 py-4 font-body text-sm leading-6 text-slate-200/95 md:rounded-[26px] md:px-6 md:py-5 md:leading-7">
              ...
              <motion.div
                ...
                className="relative mx-auto w-full max-w-xl md:float"
              >
                ...
                <div className="glass-card relative overflow-hidden rounded-[28px] border-white/15 p-5 shadow-[0_18px_56px_rgba(2,8,23,0.34)] md:rounded-[34px] md:p-8 md:shadow-[0_30px_110px_rgba(2,8,23,0.46),0_0_45px_rgba(56,189,248,0.08)]">
                  ...
                  <div className="relative">
                    ...
                    <div className="mt-6 grid gap-3 sm:grid-cols-2 md:mt-9 md:gap-4">
                      ...
                      <div className="rounded-[24px] border border-primary/20 bg-primary/10 p-5 shadow-[0_12px_32px_rgba(56,189,248,0.08)] md:rounded-[28px] md:p-6 md:shadow-[0_18px_40px_rgba(56,189,248,0.08)]">
        ...
        <motion.section className="section-container section-reveal pt-8 md:pt-18" {...reveal}>
              ...
              <div className="mt-10 grid gap-4 md:mt-14 md:gap-6 md:grid-cols-2 xl:grid-cols-4">
                {performanceCards.map((card, index) => (
                  <motion.article
                    ...
                    transition={{ duration: shouldReduceMotion ? 0.01 : isMobile ? 0.32 : 0.45, delay: shouldReduceMotion ? 0 : index * (isMobile ? 0.04 : 0.08), ease: 'easeOut' }}
        ...
        <motion.section className="section-container section-reveal pt-8 md:pt-18" {...reveal}>
          <div className="premium-surface px-5 py-10 md:px-10 md:py-16">
            <div className="absolute inset-x-10 top-0 h-px bg-gradient-to-r from-transparent via-sky-200/60 to-transparent" />
            {!isMobile ? <div className="absolute -left-12 top-10 h-44 w-44 rounded-full bg-sky-400/12 blur-3xl" /> : null}
            {!isMobile ? <div className="absolute -right-10 bottom-6 h-52 w-52 rounded-full bg-cyan-300/12 blur-3xl" /> : null}
            <div className="relative">
              <div className="mx-auto max-w-3xl text-center">
              ...
              <div className="mt-8 rounded-[26px] border border-sky-200/15 bg-sky-400/10 px-5 py-5 text-center shadow-[0_12px_36px_rgba(14,165,233,0.1)] md:mt-12 md:rounded-[30px] md:px-6 md:py-6 md:shadow-[0_18px_60px_rgba(14,165,233,0.12)]">
              ...
              <div className="mt-10 grid gap-4 md:mt-14 md:gap-6 md:grid-cols-2 xl:grid-cols-3">
                {webDevelopmentCards.map((card, index) => (
                  <motion.article
                    ...
                    transition={{ duration: shouldReduceMotion ? 0.01 : isMobile ? 0.32 : 0.45, delay: shouldReduceMotion ? 0 : index * (isMobile ? 0.03 : 0.06), ease: 'easeOut' }}
                    whileHover={cardHover}
        ...
        <motion.section id="projects" className="section-container section-reveal relative space-y-6 md:space-y-7" {...reveal}>
        ...
        <motion.section className="section-container section-reveal" id="stack" {...reveal}>
        ...
        <motion.section id="about" className="section-container section-reveal" {...reveal}>
          <div className="premium-surface p-6 md:p-12 lg:p-16">
            {!isMobile ? <div className="absolute right-0 top-0 h-44 w-44 rounded-full bg-primary/10 blur-3xl" /> : null}
            {!isMobile ? <div className="absolute left-10 bottom-6 h-40 w-40 rounded-full bg-accent/8 blur-3xl" /> : null}
            <div className="relative grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-start md:gap-10">
              ...
              <div className="grid gap-3 md:gap-4">
                {[t.aboutHighlightOne, t.aboutHighlightTwo, t.aboutHighlightThree].map((item, index) => (
                  <div key={item} className="rounded-[24px] border border-white/10 bg-slate-950/35 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] transition duration-300 ease-in-out md:rounded-[28px] md:p-6 hover:border-primary/25 hover:bg-slate-950/45">
        ...
        <motion.section id="contact" className="section-container section-reveal pt-8 pb-20 md:pt-10 md:pb-28" {...reveal}>
          <div className="relative overflow-hidden rounded-[28px] border border-primary/20 bg-[linear-gradient(135deg,rgba(2,6,23,0.96),rgba(10,20,36,0.94)_45%,rgba(8,47,73,0.92)_100%)] p-6 shadow-[0_18px_56px_rgba(2,8,23,0.36)] md:rounded-[36px] md:p-12 md:shadow-[0_30px_100px_rgba(2,8,23,0.46)] lg:p-14">
            {!isMobile ? <div className="absolute -right-10 top-0 h-48 w-48 rounded-full bg-primary/22 blur-3xl" /> : null}
            {!isMobile ? <div className="absolute left-0 top-1/3 h-40 w-40 rounded-full bg-accent/12 blur-3xl" /> : null}
